...
readonly -a DOCTOR_REQUIRED_COMMANDS=(
  awk cp curl dirname find ln mkdir mktemp mv readlink rm sha256sum sort tar xz
)

extract_reported_version() {
  local output="$1"
  if [[ "${output}" =~ ^Blender[[:space:]]+([0-9]+\.[0-9]+\.[0-9]+)([[:space:]]|$) ]]; then
    printf '%s\n' "${BASH_REMATCH[1]}"
  else
    return 1
  fi
...
usage() {
  cat <<'EOF'
Usage: blender/doctor.sh [--strict]

Diagnose the local Blender installation without changing it.
